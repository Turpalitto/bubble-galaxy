...
**«Подключено»** = реально вызывается из `play()` через `playSample()` и загружает существующий файл из `public/audio/`; при ошибке загрузки автоматически включается синтезированный fallback. Остальные эффекты без ключа в реестре продолжают использовать синтез.
